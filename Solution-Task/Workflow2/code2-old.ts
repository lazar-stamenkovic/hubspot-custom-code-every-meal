...
exports.main = async (event, callback) => {
  ...
  try {
    const lineItemId = await new Promise((resolve, reject) => {
      var options = {
        'method': 'GET',
      ...
      request(options, function (error, response) {
        ...
        if (responseData.results && responseData.results.length > 0) {
          resolve(responseData.results[0].id)
        } else {
          reject(`failed to parse line items -${JSON.stringify(responseData)} `)
        }
      });
    });
    if (!lineItemId) {
      throw Error(`can't find line item id`)
    }
    const outputFields = await new Promise((resolve, reject) => {
      var lineItemOptions = {
        'method': 'GET',
      ...
      request(lineItemOptions, function (error, lineItemResponse) {
        ...
        if (!resData || !resData.properties) {
          return reject(`failed to get line item details - ${JSON.stringify(resData)}`);
        }
